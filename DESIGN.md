...
- 2026-08-31 11:51 值守复验（P6fix 交付后例行巡检）：四门全绿（fmt/clippy 0 命中、build 过、map+types 21 测试全绿），工作区仅 thesis/ 论文润色未提交。值值守卫任务到此收尾
- 2026-08-31 12:20 P6fix 终验通过（fmt OK、clippy 0、494 测试全绿、门控代码抽查到位、typst 0 error）→ commit dea57b1。e2e 3 轮全 PASS：0.1160/0.0258/0.1381m（mean±std 0.0933±0.0595），拒绝计数集中启动期。**项目交付完成**：6 commit（b3fd676→dea57b1），三大件齐备。待办：李萨如 bench（含 void 引擎对比，用户已预约）
- 2026-08-31 12:55 VIO vs VOID 对比实测（/tmp/void_compare/）：悬停 VOID 0.0933m vs 李萨如 VOID 发散 998m（depth_ok=47/visual_ok=118，97 次更新被门控拒）、VIO 李萨如直接误差 0.594m。用户裁决：门控是自造机制（原文没有），全部删除；根因=downsample_voxel 0.5m 地图饥饿（单帧仅 ~48 点）。13:05 派 P8（PID 49803）：删 update_gate/warmup/连续拒绝全部机制 + 降采样 0.1m+max_points 2000 + 悬停 3 轮(<0.15m)与李萨如 3 轮(<0.3m 且优于 VIO 0.594m)双达标验收
