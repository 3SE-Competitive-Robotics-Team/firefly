...
FAST-LIVO2-faithful pipeline in which robustness comes from the measurement
model (per-point noise, plane uncertainty, per-point outlier rejection)
with no state-level gating. The entire system is implemented in Rust with an
...
On the 75 s end-to-end hovering experiment, VOID achieves an ATE-RMS below
0.15 m across three independent runs with the unmodified Algorithm 1
sequence (propagate → depth update → visual update → map building) applied
to every frame. The paper is structured to mirror the FAST-LIVO2 paper
section by section, so that the depth-camera adaptation can be audited
against the reference implementation.

Future work includes: (i) a full quantitative ablation campaign across
multiple trajectories and sensor configurations; (ii) motion-adaptive
measurement noise and model-based outlier criteria for aggressive flight;
(iii) rolling-shutter and auto-exposure modeling for real depth cameras;
and (iv) integration of the sliding-window map behavior into the
end-to-end evaluation on long-range trajectories.
