//! VIO 进程：MSCKF 估计器（`firefly-vio`）+ iceoryx2 zero-copy 发布 odom。
//!
//! 当前为**合成数据最小闭环**（无真实驱动）：GT 初始化 → 合成 IMU
//! （100Hz）→ 高频传播 → 10Hz 发布带 Trace ID 的 odom 消息。
//! 真实驱动接入见 TODO：RealSense 相机（realsense-rust）+ 飞控 IMU
//! （串口/PX4），届时配置相机标定与 KLT 跟踪器即可启用完整 MSCKF 更新。
//!
//! 订阅端可用 iceoryx2 的任意 subscriber 读取 `Firefly/Odometry` 话题，
//! 并以 `trace_id` 跨应用关联消息（Trace ID 中间件自动附加）。
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log::info!("VIO 进程启动：iceoryx2 zero-copy odom（Trace ID 自动附加）");
    ...
    // 真值初始化：原点静止（q = [0,0,0,1]，p/v/bg/ba = 0）
    let mut imustate = [0.0f64; 17];
    imustate[0] = 0.0;
    imustate[4] = 1.0;
    vio.initialize_with_gt(&imustate);
    log::info!("已初始化：t=0 原点静止");

    // odom 发布器（Trace ID 由中间件在 publish 时自动分配）
    let publisher = OdomPublisher::new()?;
    log::info!("已打开话题 {ODOM_TOPIC}");

    // 合成 IMU：静止悬停（比力 = +g 抵消重力）+ 轻微角速度扰动
